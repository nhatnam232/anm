...
export type WikiCharacter = {
  ...
  name: string
  /** Optional cross-link to AniList character ID — used by the main app's
      `<CharacterDetail>` page so we can offer a "View on Wiki" deep-link. */
...
export type WikiStory = {
  id: string                    // slug
  title: string
  /** Optional cross-link to a main-app anime page. */
