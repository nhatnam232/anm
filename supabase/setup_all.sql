...
-- ============================================================
-- 9. v8 — Quests / Achievements, Activity Feed, Admin Daily Password
-- (from supabase/schema_v8_quests_activity_admin.sql)
-- ============================================================
...
begin
  for q in (select id, target_value, badge_reward from public.quests
    ...
    on conflict (user_id, quest_id) do update
      set progress = user_quest_progress.progress + delta, updated_at = now();
    ...
    if current_progress >= q.target_value and not was_completed then
      update public.user_quest_progress set completed_at = now()
        where user_id = uid and quest_id = q.id;
      if q.badge_reward is not null then perform public.grant_badge(uid, q.badge_reward); end if;
    end if;
  end loop;
end;
$$;
...
create policy "activity_read_public" on public.activity_feed for select using (is_public = true);
create policy "activity_read_own"    on public.activity_feed for select using (auth.uid() = user_id);
create policy "activity_insert_own"  on public.activity_feed for insert with check (auth.uid() = user_id);
create policy "activity_delete_own"  on public.activity_feed for delete using (auth.uid() = user_id);
...
begin
  if tg_op = 'INSERT' and new.is_public = true then
    ...
    values (new.user_id, 'collection_create', new.id, jsonb_build_object('title', new.title));
  end if;
  return null;
end;
$$;
...
create or replace view public.activity_feed_enriched as
  select a.id, a.user_id, a.kind, a.anime_id, a.anime_title, a.anime_cover,
...
create index if not exists admin_passwords_date_idx on public.admin_passwords(valid_for_date desc);
...
create policy "admin_passwords_no_direct_access" on public.admin_passwords for select using (false);
...
-- ── system_secrets: auto-bootstrapping CRON_SECRET (v8.2) ───────────────────
...
create policy "system_secrets_no_direct" on public.system_secrets for select using (false);
...
begin
  ...
  from public.system_secrets where key = 'cron_secret' limit 1;
  ...
  insert into public.system_secrets (key, value, description)
    values ('cron_secret', new_secret, 'Auto-generated shared secret for cron-fired API calls')
...
declare stored_hash text; ok boolean := false;
begin
  if not public.is_mod_or_above(auth.uid()) then return false; end if;
  select password_hash into stored_hash from public.admin_passwords
    where valid_for_date = current_date limit 1;
...
--     quests, user_quest_progress, activity_feed, admin_passwords
--   • Views:  anime_like_counts, anime_reaction_counts, user_library_stats,
--             comment_vote_counts, activity_feed_enriched
...
--               quests_library_trg, quests_comments_trg,
--               activity_library_trg, activity_collection_trg
--   • Helpers: grant_badge, revoke_badge, bootstrap_owner, is_mod_or_above,
--              approve_edit, reject_edit, bump_quest_progress,
--              rotate_admin_password, verify_admin_password
-- ============================================================
